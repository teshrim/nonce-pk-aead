...
\caption{ {\bf Left:} Unpredictability notion for secret-AD algorithm~$\mdalg$. {\bf Right:} Another potential predictability notion, this one simulation-based. The mapping $\phi$ is information about~$X$ that is sufficient to simulate secret-AD production (when giving positive results).  When $\phi$ is the identity map, then the simulator can perfectly mimic secret-AD production.}\tsnote{Too strong?  Really depends on application.  But at least here we can support things like $\seciv$ being a sampled-once string like a user password.}
