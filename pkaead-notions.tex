...
\tsnote{Note that our privacy definition is different than Mihir's.  He has (potential) per-message nonce-generation, and a one-time secret nonce (``seed'').  The adversary does not get to see the nonce, but does got to influence it's choice though the ``nonce selector'', whatever that is.  Our definition gives control of the nonce to the adversary, and has (potential) per-message secret nonce (i.e. secret-AD).  I'm not sure how his and our definitions relate, formally.}

\paragraph{Authenticity. }  \tsnote{Needs to be reworked.}
