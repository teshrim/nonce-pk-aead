\section{Constructions}
\label{sec:constructions}

\paragraph{Hash-then-key. } \tsnote{We're basically using one-time signature schemes.  I'll bet this composition has appeared before.  Check.} 
\task{Prove security in the ROM.}
\begin{figure}[tbhp]
\begin{center}
\fpage{.5}{
\hpagess{.45}{.55}{
 \underline{$\Encprim{\pk}{\vad}{\vmsg,\umd}$}:\\[2pt]
 $R \gets H(\langle \pk, \vad, \vmsg, \umd \rangle)$\\ 
 $(\pk',\sk') \gets \Kgen(R)$\\
 $C \gets \encprim{\pk}{}{\langle \vmsg, \umd,\pk' \rangle}$\\
 $\sigma \gets \signprim{\sk'}{}{C}$\\
 Return $C,\sigma$
 }
 {
 \underline{$\Decprim{\sk}{\vad}{(C,\sigma),\auxinput}$}:\\[2pt]
 $\langle \vmsg, \umd, \pk' \rangle \gets \decprim{\sk}{}{C} $\\
 $ v \gets \verifyprim{\pk'}{}{C,\sigma}$\\
 if $v=1$ and $\auxinput \in \Witness_\umd$ then\\
\nudge Return $\vmsg$\\
Return $\bot$
 }
}
\caption{Hash-then-key construction.  The underlying public-key encryption $\enc$ is deterministic. }
\label{fig:HtK}
\end{center}
\end{figure}


\paragraph{Hash-then-encrypt.} \tsnote{This is the most obvious construction, and (I think) given in [BT].} \task{Prove security in the ROM.}
\begin{figure}[tbhp]
\begin{center}
\fpage{.5}{
\hpagess{.45}{.55}{
 \underline{$\Encprim{\pk}{\vad}{\vmsg,\umd}$}:\\[2pt]
 $R \gets H(\langle \pk, \vad, \vmsg, \umd \rangle)$\\ 
 $C \gets \encprim{\pk}{}{\langle \vmsg, \umd, R\rangle}$\\
 Return $C$
 }
 {
 \underline{$\Decprim{\sk}{\vad}{C,\auxinput}$}:\\[2pt]
 $\langle \vmsg, \umd \rangle \gets \decprim{\sk}{}{C} $\\
 if $\auxinput \in \Witness_\umd$ then\\
\nudge Return $\vmsg$\\
Return $\bot$
 }
}
\caption{Hash-then-encrypt construction.  The underlying public-key encryption $\enc$ is deterministic.   }
\label{fig:HtE}
\end{center}
\end{figure}

\paragraph{KEM-DEM instantiation of HtE.} 
\task{Prove security in the ROM when the underlying symmetric encryption is an AEAD scheme.}
\task{Does it add anything if we formalize KEMs to take AD?  }

\begin{figure}[tbhp]
\begin{center}
\fpage{.5}{
 \hpagess{.45}{.55}{
 \underline{$\Encprim{\pk}{\vad}{\vmsg,\umd}$}:\\[2pt]
 $R \gets H(\langle \pk,\vad,\vmsg,\umd \rangle)$\\ 
 $(K,\wrap) \gets \encapprim{\pk}{}{R}$\\
 $\vad' \gets (\vad,\wrap)$\\
 $C \gets \encprim{K}{\vad'}{\langle \vmsg, \umd \rangle}$\\
 Return $(\wrap,C)$
 }
 {
 \underline{$\Decprim{\sk}{\vad}{(\wrap,C),\auxinput}$}:\\[2pt]
$K \gets \decapprim{\sk}{}{\wrap}$\\[3pt]
$\vad' \gets (\vad,\wrap)$\\
$\langle \vmsg,\umd \rangle \gets \decprim{K}{\vad'}{C}$\\ 
if $\auxinput \in \Witness_\umd$ then\\
\nudge Return $\vmsg$\\
Return $\bot$
 }
}
%
%\medskip
%\hspace*{.5ex}\fpage{.5}{
% \hpagess{.45}{.55}{
% \underline{$\encapprim{\pk}{\vad}{R}$}:\\[2pt]
%$(K,X) \gets \OLencapprim{\pk}{}{\langle \vad,R}\rangle$\\
% Return $(K,X)$
% }
% {
% \underline{$\decapprim{\sk}{\vad}{X}$}:\\[2pt]
% $\langle K,\vad'\rangle \gets \OLdecapprim{\sk}{}{X}$\\
% if $(\vad')=(\vad)$ then \\
%\nudge Return $K$ \\
%Return $\bot$
% } 
%} 
\caption{KEM-DEM instantiation of HtE.  The underlying symmetric encryption $\enc$ is deterministic and supports associated data. The unspecified encapsulation algorithm $\encap$ can be any randomized algorithm with its internal randomness replaced by~$R$.  
}
\label{fig:kem-dem-HtE}
\end{center}
\end{figure}

\paragraph{Constructions from standard-model assumptions. } \task{Find!}


