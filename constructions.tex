...
This particular construction allows for recovery of~$\overline{S}$ upon decryption.  In turn, this allows a re-encryption authenticty check on the decapsulated key~$K$, in addition to any authenticity checks on the ciphertext~$C$ that the AEAD scheme imposes.  \tsnote{The auth check on~$K$ is indirect, and we'll need to nail down what properties of the AEAD scheme are needed.}  \tsnote{Need to update KEM syntax to match figure.}  By having decryption return $M \concat \overline{S}$, we might enable higher-level checks.  For example, if the senders secret IV~$S$ has been previously stored receiver-side, or transmitted out-of-band.  Note that the secret IV is not required to change across encryption calls, so $S=H(\mbox{password}\concat\mbox{salt})$ may be an option. \tsnote{Need to patch up PKE syntax and proper operation requirement, if decryption returns something other than the plaintext that was encrypted (or $\bot$)}
...
\fpage{.5}{
 ...
 $(K,X,\overline{\seciv}) \gets \encapprim{\pk}{\header,\pubiv}{\overline{\seciv}}$\\
 ...
 \hspace*{3ex} Return $\bot$\\
 Return $(M \concat \overline{\seciv})$\\
 }
}
