...
\item\emph{Why does decryption take $\pubiv,\header$ as input?} \task{Need to justify this choice.  Mihir explicitly did not provide $\pubiv,\header$ to decryption.}
...
Allowing $\mdalg$ to pass state to itself admits things like secret counters.  Additionally allowing $\mdalg$ to be randomized captures things such as \task{This needs thought.  What kind of metadata do we want to allow?  Currently it can only depend on the message and the state.  If the state is initialized to $\emptystring$, this means that the output of $\mdalg(\st,M)$ can only depend upon the content of the current string~$M$ and the contents of any strings $M_1,M_2,\ldots$ that were previously seen by~$\mdalg$.  Is this sufficient?  Let's think about the implications.  If we think of~$M$ as ``the plaintext application content encrypted along with the sensitive metadata~$S$'', then we can't capture~$S$ that depend on non-content information (e.g. operating system information, call graphs, etc).  Hmmm... actually maybe this is okay, if we don't think of the state as being initially empty.  $\mdalg$ isn't a real thing, it isn't necessarily an algorithm that one needs to implement as part of the development of the encryption scheme.  It's abstracting away whatever ``process'' creates metadata for content.  So a lot depends on what is the initial state.
}

\tsnote{State also allows for stateful distributions.  Note that the distribution might not depend on $M_b$ at all, e.g. when~$S$ contains a password.}
