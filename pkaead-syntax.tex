...
For proper operation, we require that for all $\header\in\adata$, all $(\pk,\sk)\in\pubkeys\times\seckeys$ that may be output by $\Kgen(\kreg)$ with non-zero probability, and for all $\pubiv\in\pubivs$, $\seciv\in\secivs$, and $M\in\ptxts$, we have $\Decprim{\sk}{\header,\pubiv}{\Encprim{\pk}{\header,\pubiv}{S,M}}=M$.

\emph{Decryption without secret-AD recovery} is 
a mapping $\Dec \colon \seckeys \times \adata \times \pubivs \times \ctxts \times \bits^*\to \ptxts \cup \{\bot\}$.  As before, we write $M \gets \Decprim{\sk}{\header,\pubiv}{C,\auxinput}$ for the operation of decryption.  
For proper operation, we require that for all $\header\in\adata$, all $(\pk,\sk)\in\pubkeys\times\seckeys$ that may be output by $\Kgen(\header)$ with non-zero probability, and for all $\pubiv\in\pubivs$, $\seciv\in\secivs$, $\auxinput\in\bits^*$ and $M\in\ptxts$, we have $\Decprim{\sk}{\header,\pubiv}{\Encprim{\pk}{\header,\pubiv}{S,M},\auxinput}=(S,M)$. \tsnote{The auxilary input~$\auxinput$ is meant to support authentication \textit{within} the decryption boundary.  For example, when~$\auxinput$ is something like the receiver-side stored version of~$S$.  Not sure how~$\auxinput$ should affect proper operation.  Should the above statement only hold for $\auxinput$ ``equivalent'' to~$S$?  Should correctness be stated as a security property, e.g. a game where the adversary attempts to forge a~$\auxinput$ that causes decryption to output $(S,M)\neq\bot$.}
