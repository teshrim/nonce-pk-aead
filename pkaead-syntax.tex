...
Allowing $\mdalg$ to pass state to itself admits things like secret counters.  Additionally allowing $\mdalg$ to be randomized captures things such as \task{This needs thought.  What kind of metadata do we want to allow?  Currently it can only depend on the message.  Should we be allowing for and protecting other thing?  User-related metadata, say?  Perhaps we can assume (without loss?) that the string~$M$ may have encoded into it, say, user and context-related data.} \tsnote{State also allows for stateful distributions.  Note that the distribution might not depend on $M_b$ at all, e.g. when~$S$ contains a password.}
