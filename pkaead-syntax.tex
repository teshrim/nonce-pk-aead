...
For example, $\umd$ might be a ``lightweight'' secret~$s$ (e.g. a password, a unique device ID), and $\auxinput=\langle \mathrm{salt},H(\mathrm{salt} \concat s) \rangle$, allowing authenticity checks of the form \emph{this plaintext was sent by this user/device}.\tsfnote{How does the receiver know which $\mathrm{salt},H(\mathrm{salt}\concat\mathrm{pwd})$ to pass in?  The user/device name could be part of the public-AD.  In some settings this might not be acceptable.} \task{List other examples here, e.g., I2P garlic routing checks, QUIC-like protocols that bind a server secret to an IP-address, etc.}
%
...
The latter viewpoint is the one taken by Bellare and Tackmann (BT)~\cite{mihir}, where nonce-based PKE is viewed as a method for hedging against bad randomness generation.
...
\paragraph{Comparison with BT~\cite{mihir}.}
