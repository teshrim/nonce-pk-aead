...
Allowing $\mdalg$ to pass state to itself admits things like secret counters.  Additionally allowing $\mdalg$ to be randomized captures things such as \tsnote{...Need to flesh this out more.} \tsnote{State also allows for stateful distributions.  Note that the distribution might not depend on $M_b$ at all, e.g. when~$S$ contains a password.}
