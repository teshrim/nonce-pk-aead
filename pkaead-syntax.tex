...
For proper operation, we require that for all $\header\in\adata$, all $(\pk,\sk)\in\pubkeys\times\seckeys$ that may be output by $\Kgen(\header)$ with non-zero probability, and for all $\pubiv\in\pubivs$, $\seciv\in\secivs$, and $M\in\ptxts$, we have $\Decprim{\sk}{\header,\pubiv}{\Encprim{\pk}{\header,\pubiv}{S,M}}=(S,M)$. \tsnote{To support actual AE-like behaviors, decryption needs to take an additional string~$Z$ as input, which is meant to be something like the receiver-side stored version of~$S$.  Should fix this.}
