...
%For proper operation, we require that for all $\header\in\adata$, all $(\pk,\sk)\in\pubkeys\times\seckeys$ that may be output by $\Kgen(\header)$ with non-zero probability, and for all $\pubiv\in\pubivs$, $\seciv\in\secivs$, $\auxinput\in\bits^*$ and $M\in\ptxts$, we have $\Decprim{\sk}{\header,\pubiv}{\Encprim{\pk}{\header,\pubiv}{S,M},\auxinput}=(S,M)$. 
\tsnote{The auxilary input~$\auxinput$ is meant to support authentication \textit{within} the decryption boundary.  For example, when~$\auxinput$ is something like the receiver-side stored version of~$S$.  Not sure how~$\auxinput$ should affect proper operation.  Should there be only one~$\auxinput$ for each~$\seciv$?  A class $\mathcal{Z_\seciv}\subseteq\bits^*$ of valid~$\auxinput$?}
